 ...
 *
 *
 * 10.21刘佳朋修改该函数，删除了原先显示项目名称的逻辑，修改了plugin.xml中插件名称
 */
public class ShowProjectNameAction extends AnAction {
     ...
     *
     * 10.21刘佳朋修改部分代码，
     *                1.之前的代码会出现ref HEAD cannot be resolved的报错和 main分支不存在的报错，一开始不会有main分支，有master分支
     *                2.点击两次会出现new-feature-branch已存在的报错，应先检查是否有new-feature-branch再创建
     */
    private void createBranchAndCommitChanges(Project project) {
        try {
            ...
            if (basePath != null) {
                ...
                // 先把更改提交才可以创建分支否则会出现ref HEAD cannot be resolved的报错
...
//                    System.out.println("分支名称:" + ref.getName());
//                }

                // 检查分支是否存在
                boolean newBranchExists = branches.stream().anyMatch(ref -> ref.getName().equals("refs/heads/" + newBranchName));
                ...
                boolean mainBranchExists = branches.stream().anyMatch(ref -> ref.getName().equals("refs/heads/" + originalBranch));

                if (!mainBranchExists) {
                    // 创建并切换到新的分支
                    git.branchCreate().setName(originalBranch).call();
                }
                // 切换回主分支
